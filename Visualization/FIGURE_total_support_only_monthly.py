...
metrics = ['_Potential', '_CE','_CE_social']
...
def adapted_box_plot_on_ax(data_dict, time, scenario, matching, ax,
    ...
    if colors is None:
        colors = {"Cash-Equivalent (9.3%)": "#b8f4ff", "Potential (2%)": "#5c5c5c", "Cash-Equivalent (2%)": "#8aa5aa"}
    ...
    cash_eq__social_cols = [col for col in data.columns if "_CE_social" in col]

    # Create a DataFrame to store data for box plotting
    box_plot_data = []

    # Add data for potential and cash-equivalent
    for ce_col, soc_pol, pot_col in zip(cash_eq_cols, cash_eq__social_cols, potential_cols):

        label = pot_col.split("_")[1]  # Extract technology
        box_plot_data.append(pd.DataFrame(
            {'Value': data[ce_col], 'Group': [label] * len(data), 'Type': ["Cash-Equivalent (9.3%)"] * len(data)}))
        box_plot_data.append(pd.DataFrame(
            {'Value': data[soc_pol], 'Group': [label] * len(data), 'Type': ["Cash-Equivalent (2%)"] * len(data)}))
        box_plot_data.append(
            pd.DataFrame({'Value': data[pot_col], 'Group': [label] * len(data), 'Type': ["Potential (2%)"] * len(data)}))

    # Concatenate the DataFrames
    box_plot_data = pd.concat(box_plot_data)

    # Create the box plot
    sns.boxplot(x='Group', y='Value', hue='Type', data=box_plot_data, ax=ax, palette=colors, width=0.6)
    ax.set_xlabel('')
    ...
    ax.set_ylabel(r'Tax Credits [$\frac{2023\$}{Kg \ H_2} $]'+f',{time+3}' if left else '', fontsize=16, fontweight='bold')
...
plt.savefig(f"FINAL final figures/TC Yearly.png", dpi=500)
